#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Trading Economics에서 폴리에틸렌(PE) 선물 스팟 가격을 매일 1회 크롤링해
data/pe_futures_spot.csv 에 append한다.

같은 날짜의 행이 이미 있으면 아무것도 하지 않는다(idempotent) — 크론이
같은 날 여러 번 돌아도 중복이 쌓이지 않는다.

원본 로직은 로컬 프로젝트의 etl/trading_economics.py 와 동일:
- tradingeconomics.com/commodity/polyethylene 무료 페이지를 파싱
- 다롄상품거래소(DCE) 폴리에틸렌 선물, CNY/T
...
CSV_PATH = Path(__file__).parent.parent / "data" / "pe_futures_spot.csv"
...
def fetch_spot():
    ...
    if "<title>\n\tTRADING ECONOMICS" in html and "polyethylene" not in html.lower()[:2000]:
        raise RuntimeError("상품 페이지가 존재하지 않음 (홈으로 폴백됨)")
    ...
    if m:
        ...
        if not m:
            raise RuntimeError("가격 문구 파싱 실패 — TE가 페이지 문구를 바꿨을 가능성")
...
def append_if_new(row: dict) -> bool:
...
if __name__ == "__main__":
    row = fetch_spot()
    added = append_if_new(row)
    status = "추가됨" if added else "이미 존재 (스킵)"
    print(f"[{status}] {row['date']}  {row['value']:,.2f} {row['unit']}  "
